...
Here is an example of how to use **BreastSubtypeR** for breast cancer subtyping using Multiple Methods (Multi-Method Subtyping Functionality). By default, the user manually selects the methods to be used:
```{r}
...
Here is an example of how to use **BreastSubtypeR** for breast cancer subtyping using Multiple Methods with **AUTO** mode feature enabled. AUTO mode automatically selects methods based on the ER/HER2 distribution of the test cohort:
```{r}
...
Here is an example of how to use **BreastSubtypeR** for breast cancer subtyping with parker.original method:   
```{r}
...
| Approach | Usage |
|-----------------|-----------------|
| Multi-Method subtyping: User-Selected Methods | BS_Multi(methods = c("parker.original", "ssBC.v2", "sspbc",...), ...) |
| Multi-Method subtyping: AUTO mode enabled | BS_Multi(methods = "AUTO", ...) |
| Single-Method subtyping: parker.original   |  BS_parker(calibration = "Internal", internal = "medianCtr" ,...) |
| Single-Method subtyping: genefu.scale  | BS_parker(calibration = "Internal", internal = "meanCtr" ,...) |
| Single-Method subtyping: genefu.robust  | BS_parker(calibration = "Internal", internal = "qCtr",...) |
| Single-Method subtyping: cIHC | BS_cIHC(...) |
| Single-Method subtyping: cIHC.itr    |  BS_cIHC.itr(...) |
| Single-Method subtyping: PCAPAM50   |  BS_PCAPAM50(...) |
| Single-Method subtyping: ssBC    |  BS_ssBC(s = "ER", ...) |
| Single-Method subtyping: ssBC.v2   | BS_ssBC(s = "ER.v2", ...) |
| Single-Method subtyping: AIMS   | BS_AIMS(...) |
| Single-Method subtyping: sspbc | BS_sspbc(...) |
