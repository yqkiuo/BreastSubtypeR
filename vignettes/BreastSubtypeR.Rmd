...
| `PCAPAM50` | PCA-based iterative PAM50; ESR1 expression used for ER balancing | NC-based | [Raj-Kumar et al., 2019](https://doi.org/10.1038/s41598-019-44339-4) |
...
# Preprocessing: automatically apply tailored normalization, map probes/IDs to Entrez and (optionally) impute missing values
data_input <- Mapping(
...
The app enables:\
- Upload expression, clinical, and feature-annotation tables.
- Run single methods or `AUTO` (assumption-aware selection).\
- Choose 5-class (incl. Normal-like) or 4-class.\
- View per-sample concordance (entropy), heatmap & pie summaries.\
- Export Calls only or Full metrics (ROR available for NC methods when `TSIZE`/`NODE` are present and numeric).
...
Yang Q., Hartman J., Sifakis E.G. *BreastSubtypeR: A Unified R/Bioconductor Package for Intrinsic Molecular Subtyping in Breast Cancer Research*. **NAR Genomics and Bioinformatics** (2025). **Editor’s Choice**. https://doi.org/10.1093/nargab/lqaf131
