...
data <- OSLO2MEIT0obj$OSLO2EMIT0.103.genematrix_noNeg.subset
data_input <- Mapping(gene_expr = data, featuredata = OSLO2MEIT0obj$anno_feature.subset, impute = TRUE, verbose = TRUE )

# Run the subtyping
methods <- c("parker.median", "PCAPAM50", "sspbc")
result <- BS_Multi(data_input = data_input, pheno = OSLO2MEIT0obj$clinic.oslo, methods = methods, Subtype = TRUE)
...
Here is an example of how to use **BreastSubtypeR** for breast cancer subtyping with the `parker.original` method:   
...
Here is an example of how to use **BreastSubtypeR** for breast cancer subtyping with the `AIMS` method:   
...
genes <- genes.s$EntrezGene.ID[which(genes.s$AIMS == "Yes")]
...
We welcome contributions to the package. If you find any bugs or have feature 
requests, feel free to open an issue [here](https://github.com/yqkiuo/BreastSubtypeR/issues).
