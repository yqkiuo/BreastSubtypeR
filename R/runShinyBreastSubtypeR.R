...
#' Step 1:   
#' 
#' The input data can be loaded from the user's workspace or by selecting a CSV/text
#' file for the expression data, a CSV/text file for clinical information, and
#' a CSV/text file for feature annotations.    
#' 
#' After loading the necessary files, users can click the "Map Now" button once and wait for the notification. 
#' If the Mapping() function runs successfully, a message will appear stating, "You may now proceed to Step 2."
#' 
#' 
#' 
#' Step 2:  
#' 
#' Users can select the desired subtyping method and adjust the relevant parameters to conduct their analysis.
#' Once the analysis is complete, a message will indicate, "Analysis is complete."
...
iBreastSubtypeR = function() {
  ...
  if (any(!maskshinydeps))
    stop(sprintf("Please install the following packages to use the BreastSubtypeR app:\n\n  %s\n",
